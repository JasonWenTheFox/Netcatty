...
test("BusyBox-resolved login shell falls back to its original alias and probes once", async () => {
  let probes = 0;
  const session = { protocol: "ssh" };
  const probe = async () => {
    probes += 1;
    return (
      `${PROBE_OUTPUT_MARKER}/bin/busybox\n`
      + `${PROBE_OUTPUT_MARKER}/bin/ash\n`
    );
  };
...
test("probe command is fish-parseable and forces POSIX sh", () => {
  ...
  assert.equal(
    command.match(new RegExp(PROBE_OUTPUT_MARKER, "g"))?.length,
    2,
    "probe must preserve the original alias after the remotely resolved path",
  );
