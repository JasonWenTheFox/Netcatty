...
test("real bash startup completion marker restores agent availability only after the prompt", {
  skip: !existsSync("/bin/bash"),
}, async () => {
  const expectedPrompt = "user@host:~$";
  ...
  try {
    await waitForPtyText(ptyProcess, expectedPrompt);
    const input = `true\rprintf '\\n%s%s\\n' '${left}' '${right}'\r`;
    trackSessionPendingUserInput(session, input, { automated: true });
    session._automatedInputCompletionMarker = marker;
    ...
    assert.equal(capturePendingInputState(session).pending, true);

    const markerThenPrompt = new Promise((resolve, reject) => {
      let output = "";
      const timeoutId = setTimeout(() => {
        disposable.dispose();
        reject(new Error(`Timed out waiting for completion marker and prompt\n${output}`));
      }, 5000);
      const disposable = ptyProcess.onData((chunk) => {
        output += chunk;
        const markerIndex = output.indexOf(marker);
        if (markerIndex === -1 || !output.slice(markerIndex + marker.length).includes(expectedPrompt)) return;
        clearTimeout(timeoutId);
        disposable.dispose();
        resolve(output);
      });
    ...
    await markerThenPrompt;

    assert.equal(capturePendingInputState(session).pending, false);
    assert.equal(session.lastIdlePrompt, expectedPrompt);
  } finally {
    outputTracker.dispose();
    try { ptyProcess.kill(); } catch {}
  }
...
test("real bash foreground startup command cannot fake automated completion", {
  skip: !existsSync("/bin/bash"),
}, async () => {
  const expectedPrompt = "user@host:~$";
  const marker = "__NCAUTO_0123456789abcdef0123456789abcdef__";
  ...
  const left = marker.slice(0, midpoint);
  const right = marker.slice(midpoint);
  const completionCommand = `printf '\\n%s%s\\n' '${left}' '${right}'`;
  const input = `cat\r${completionCommand}\r`;
  const session = {};
  const ptyProcess = nodePty.spawn("/bin/bash", ["--noprofile", "--norc", "-i"], {
    cols: 120,
    rows: 30,
  ...
  try {
    ...
    const commandConsumedByCat = waitForPtyText(ptyProcess, completionCommand);
    ptyProcess.write(input);
    await commandConsumedByCat;
    assert.equal(capturePendingInputState(session).pending, true);
    assert.equal(session._automatedInputCompletionSeen, false);
  } finally {
    outputTracker.dispose();
    try { ptyProcess.kill(); } catch {}
  }
