...
export const useSettingsState = (options: { enableSettingsSync?: boolean; enableSystemEffects?: boolean } = {}) => {
  ...
  const [sftpFolderPrescan, setSftpFolderPrescan] = useState<boolean>(() =>
    resolveSftpFolderPrescanEnabled(() => localStorageAdapter.readBoolean(STORAGE_KEY_SFTP_FOLDER_PRESCAN)),
  );
  ...
  const rehydrateAllFromStorage = useCallback(() => {
    ...
    const storedFolderPrescan = localStorageAdapter.readBoolean(STORAGE_KEY_SFTP_FOLDER_PRESCAN);
    if (storedFolderPrescan != null) setSftpFolderPrescan(storedFolderPrescan);
  ...
  useEffect(() => {
    localStorageAdapter.writeBoolean(STORAGE_KEY_SFTP_FOLDER_PRESCAN, sftpFolderPrescan);
    if (!persistMountedRef.current) return;
    notifySettingsChanged(STORAGE_KEY_SFTP_FOLDER_PRESCAN, sftpFolderPrescan);
  }, [sftpFolderPrescan, notifySettingsChanged]);
  ...
  return {
    ...
    settingsVersion: useMemo(() => Math.random(), [
      ...
      sftpDoubleClickBehavior, sftpAutoSync, sftpShowHiddenFiles, sftpUseCompressedUpload, sftpFolderPrescan, sftpSkipUnchanged, sftpAutoOpenSidebar, sftpFollowTerminalCwd, sftpDefaultViewMode,
