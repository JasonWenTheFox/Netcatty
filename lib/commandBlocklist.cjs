...
const DEFAULT_COMMAND_BLOCKLIST = [...COMMON_PATTERNS, ...POSIX_PATTERNS, ...POWERSHELL_PATTERNS];
...
function selectDefaultBlocklistPatterns(shellKind) {
  switch (String(shellKind || "").toLowerCase()) {
    case "powershell":
      return [...COMMON_PATTERNS, ...POWERSHELL_PATTERNS];
    case "cmd":
      return [...COMMON_PATTERNS];
    case "posix":
    case "fish":
      return [...COMMON_PATTERNS, ...POSIX_PATTERNS];
    default:
