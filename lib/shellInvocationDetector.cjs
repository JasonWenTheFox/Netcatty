...
const MAX_NESTED_SCAN_STEPS = 256;
const MIN_NESTED_SCAN_WORK = 1_000_000;
...
function pipelineInputCommand(tokens, commandStartIndex) {
  const pipeIndex = commandStartIndex - 1;
  if (tokens[pipeIndex]?.type !== "operator" || tokens[pipeIndex].value !== "|") return null;
  let startIndex = pipeIndex - 1;
  while (
    startIndex >= 0
    && (tokens[startIndex].type !== "operator" || tokens[startIndex].value === "|")
  ) {
    startIndex -= 1;
  }
  const command = reconstructWords(tokens, startIndex + 1, pipeIndex);
  return command || null;
}

function shellReadsPipedCommands(tokens, executableIndex, shellKind, nestedCommand) {
  if (shellKind === "powershell" && nestedCommand === "-") return true;
  if (nestedCommand) return false;

  const args = tokens
    .slice(executableIndex + 1, commandEndIndex(tokens, executableIndex))
    .filter((token) => token.type === "word")
    .map((token) => token.value);
  if (args.length === 0) return true;
  if (shellKind === "posix" && args.some((value) => /^-[^-]*s[^-]*$/.test(value))) return true;
  return args.every((value) => value.startsWith("-") || (shellKind === "cmd" && value.startsWith("/")));
}
...
function nestedCommandArgument(tokens, executableIndex, shellKind) {
  ...
  for (let index = executableIndex + 1; index < tokens.length && !isBoundary(tokens[index]); index += 1) {
    ...
    const inlineSeparator = shellKind === "powershell" ? rawToken.indexOf(":") : -1;
    const rawOption = (inlineSeparator >= 0 ? rawToken.slice(0, inlineSeparator) : rawToken).toLowerCase();
    const inlineValue = inlineSeparator >= 0 ? rawToken.slice(inlineSeparator + 1) : null;
    const option = shellKind === "powershell" && rawOption.startsWith("/")
      ? `-${rawOption.slice(1)}`
      : rawOption;
    const isEncodedOption = shellKind === "powershell"
      && (option === "-enc" || (option.length >= 5 && "-encodedcommand".startsWith(option)));
    const isCommandOption = shellKind === "powershell"
      ? isEncodedOption || option === "-c" || (option.length >= 4 && "-command".startsWith(option))
      : option === "--command" || (/^-[^-]+$/.test(option) && option.slice(1).includes("c"));
    if (isCommandOption) {
      const argument = inlineValue ?? tokens[index + 1]?.value;
      if (argument == null) continue;
      if (isEncodedOption) {
        return decodePowerShellEncodedCommand(argument);
      }
      if (shellKind === "powershell" && inlineValue != null) {
        return `${inlineValue} ${reconstructWords(tokens, index + 1)}`.trim();
      }
      if (shellKind === "powershell" && !tokens[index + 1].quoted) {
...
function detectShellInvocations(command, shellKind, scanState = null) {
  const kind = normalizeShellKind(shellKind);
  if (!command || !["posix", "powershell", "cmd"].includes(kind)) return [];

  const source = String(command);
  const state = scanState || {
    remainingSteps: MAX_NESTED_SCAN_STEPS,
    remainingWork: Math.max(MIN_NESTED_SCAN_WORK, source.length * 16),
    overflowReported: false,
  };
  if (state.remainingSteps <= 0 || state.remainingWork < source.length) {
    if (state.overflowReported) return [];
    state.overflowReported = true;
    return [{ group: "overflow", command: "" }];
  }
  state.remainingSteps -= 1;
  state.remainingWork -= source.length;

  const tokens = tokenizeCommand(source, kind);
  const invocations = [];

  for (const token of tokens) {
    if (kind !== "powershell" || token.type !== "word") continue;
    for (const embeddedCommand of token.embeddedCommands) {
      invocations.push(...detectShellInvocations(embeddedCommand, "powershell", state));
    }
  }
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (resolved?.nestedCommand) {
      ...
      invocations.push(...detectShellInvocations(resolved.nestedCommand, "posix", state));
    } else if (resolved?.childCommand) {
      invocations.push(...detectShellInvocations(resolved.childCommand, "powershell", state));
    } else if (resolved?.kind === "cmd") {
      const nestedCommand = cmdNestedCommand(tokens, resolved.index);
      if (nestedCommand) {
        invocations.push(...detectShellInvocations(nestedCommand, "cmd", state));
      }
      ...
      const pipelineInput = pipelineInputCommand(tokens, index);
      const scope = pipelineInput && shellReadsPipedCommands(tokens, resolved.index, resolved.kind, nestedCommand)
        ? pipelineInput
        : nestedCommand || reconstructWords(tokens, resolved.index);
      ...
      if (nestedCommand) {
        invocations.push(...detectShellInvocations(nestedCommand, resolved.kind, state));
      }
      if (isWsl) {
        const wslCommand = wslNestedCommand(tokens, resolved.index);
        if (wslCommand) {
          invocations.push({ group: "native", command: wslCommand.command });
          invocations.push(...detectShellInvocations(wslCommand.parsedCommand, "posix", state));
        }
      }
    }
    atCommandStart = false;
...
function detectInvokedShellGroups(command, shellKind) {
  return [...new Set(
    detectShellInvocations(command, shellKind)
      .map(({ group }) => group)
      .filter((group) => group === "posix" || group === "powershell" || group === "cmd"),
  )];
}
